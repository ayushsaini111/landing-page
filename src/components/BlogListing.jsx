...
export default function BlogVideoListingPage({ searchQuery = "", onClearSearch }) {
    ...
    const isInitialLoading = items.length === 0 && !error;
    ...
    const CACHE_DURATION_MAIN = 5 * 60 * 1000; // 5 minutes
    const CACHE_DURATION_SEARCH = 15 * 60 * 1000; // 15 minutes
    useEffect(() => {
        const saveState = () => {
            if (items.length > 0) {
    ...
    }, [items, page, hasMore, total, hasSearched, activeQuery]);
    ...
    useEffect(() => {
        window.refreshBlogList = () => {
            ...
            setPage(1);
            setItems([]);
            fetchData(1, false, activeQuery, true);
        };

        return () => {
            delete window.refreshBlogList;
        };
    }, [activeQuery]);
    ...
    return (
        <div className="w-full">
            ...
            {/* SEARCH RESULTS INFO */}
            {hasSearched && activeQuery && items.length > 0 && (
            ...
            {/* NO RESULTS */}
            {activeQuery && items.length === 0 && !error && (
                    ...
                    {items.map((item) => (
                        <div
                            ...
                            onClick={() => {
                                ...
                                sessionStorage.setItem("blog-card", item._id);
...
//       saveState();
//     };
//   }, [items, page, total, hasMore, searchQuery]);
...
//   useEffect(() => {
//     const savedState = sessionStorage.getItem("blog-listing-state");
//     const shouldRestore = sessionStorage.getItem("blog-card");

//     if (savedState && shouldRestore) {
//       try {
...
//           setHasSearched(!!state.searchQuery);
//           return;
//         }
//       } catch (err) {
...
//       const res = await fetch(
//         `${process.env.NEXT_PUBLIC_BASE_URL}/api/blog?page=${pageNum}&limit=${limit}${queryParam}&_t=${cacheWindow}`,
//         { signal: abortRef.current.signal, cache: "force-cache" }
//       );
...
//       /* ---------------------------------------------------------------------- */
//       /*                    SORTING (LATEST FIRST + SEARCH RANKING)             */
//       /* ---------------------------------------------------------------------- */
...
//           <Button onClick={resetAndLoadAll} variant="ctaAccent">
//             Explore More
//           </Button>
//         </div>
//       )}
...
//               key={item._id}
//               onClick={() => {
//                 sessionStorage.setItem("blog-scroll", window.scrollY);
//                 sessionStorage.setItem("blog-card", item._id);
//                 router.push(`/blog/${item._id}`);
//               }}
//             >
...
//       {hasMore && items.length > 0 && (
//         <div className="flex justify-center mt-12">
//           <Button onClick={handleLoadMore} disabled={loadingMore} variant="ctaAccent">
//             {loadingMore ? "Loading..." : "Load More"}
//           </Button>
//         </div>
//       )}
