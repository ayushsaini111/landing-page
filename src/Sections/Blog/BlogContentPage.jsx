...
export default function BlogContentPage({ slug }) {
  ...
  useEffect(() => {
    ...
    async function fetchBlog() {
      ...
      try {
        ...
        const fetchUrl = `${baseUrl}/api/blog/slug/${slug}`;
        ...
        if (!blogData) {
          throw new Error("Missing blog data in response");
        }
  ...
  if (loading) {
    return (
      <main className="w-full min-h-screen bg-background">
        <div className="bg-linear-to-b from-accent-light to-background border-b-2 border-accent-main/20">
          <div className="max-w-5xl mx-auto px-s64 py-s32">
            <div className="h-6 w-32 bg-accent-main/20 rounded animate-pulse mb-s24"></div>
            <div className="h-8 w-64 bg-accent-main/30 rounded-full mb-s16 animate-pulse"></div>
            <div className="h-12 w-4/5 bg-primary-main/20 rounded mb-s24 animate-pulse"></div>
    ...
    return (
      <main className="w-full min-h-screen bg-background mt-24">
        <div className="max-w-5xl mx-auto px-s64 py-s32">
          <button
            onClick={() => router.back()}
            className="inline-flex items-center gap-2 text-accent-main hover:text-accent-dark transition-colors"
          >
  ...
  return (
    <main className="w-full min-h-screen bg-background mt-24">
      ...
      <div className="max-w-5xl mx-auto flex flex-col gap-4 px-4 sm:px-6 md:px-12 lg:px-0 py-s32">
        ...
        {/* Title */}
        <h1 className="hero-h1 text-primary-main wrap-break-work">
          {blog.title}
        </h1>
            ...
            >
              {showCopied ? (
                <>
                  ...
                  <span className="body-small">Copied!</span>
                </>
              ) : (
                <>
