...
export async function generateMetadata() {
  ...
  const data = Data?.subServices?.propertyTransfer ?? null;

  const title = data ? `${data.title} — Arshiv Legal` : 'Property Transfer & Registration — Arshiv Legal';
  const description =
    data?.description ||
  ...
  return {
    title,
    description,
    alternates: { canonical: canonicalize('/services/property-transfer-&-registration') },
    keywords: [
      'property transfer',
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${SEO_CONFIG.siteUrl}/images/property-transfer-og.jpg`,
          width: 1200,
          height: 630,
    ...
    twitter: {
      ...
      title,
      description,
      images: [`${SEO_CONFIG.siteUrl}/images/property-transfer-og.jpg`],
    },
...
export default async function Page() {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: data.title, url: canonicalize('/services/property-transfer-&-registration') },
  ]);
