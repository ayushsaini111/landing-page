...
export async function generateMetadata() {
  ...
  const data = Data?.subServices?.publicMatters ?? null;

  const title = data ? `${data.title} — Arshiv Legal` : 'Public Interest Related Matters — Arshiv Legal';
  const description =
    data?.description ||
    'Public interest related matters: legal remedies and guidance for issues affecting the community, governance, and public welfare.';

  return {
    title,
    description,
    alternates: { canonical: canonicalize('/services/public-interest-related-matters') },
    keywords: [
      'public interest',
      'public law',
      'PIL',
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${SEO_CONFIG.siteUrl}/images/public-interest-og.jpg`,
          width: 1200,
          height: 630,
    ...
    twitter: {
      ...
      title,
      description,
      images: [`${SEO_CONFIG.siteUrl}/images/public-interest-og.jpg`],
    },
...
export default async function Page() {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: data.title, url: canonicalize('/services/public-interest-related-matters') },
  ]);
