// /src/app/services/writ-petitions/page.jsx
...
export async function generateMetadata() {
  ...
  const data = Data?.subServices?.writPetitions ?? null;

  const title = data ? `${data.title} — Arshiv Legal` : 'Writ Petitions — Arshiv Legal';
  const description =
    data?.description ||
    'Writ petitions: file habeas corpus, mandamus, certiorari, prohibition or quo warranto to protect fundamental rights and challenge unlawful state actions.';

  return {
    title,
    description,
    alternates: { canonical: canonicalize('/services/writ-petitions') },
    keywords: [
      'writ petition',
      'habeas corpus',
      'mandamus',
    ...
    openGraph: {
      title,
      description,
      url: canonicalize('/services/writ-petitions'),
      siteName: SEO_CONFIG.siteName,
      type: 'article',
      images: [
        {
          url: `${SEO_CONFIG.siteUrl}/images/writ-petitions-og.jpg`,
          width: 1200,
          height: 630,
          alt: 'Writ Petitions - Arshiv Legal',
        },
      ],
    },
    twitter: {
      ...
      title,
      description,
      images: [`${SEO_CONFIG.siteUrl}/images/writ-petitions-og.jpg`],
    },
...
export default async function Page() {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: data.title, url: canonicalize('/services/writ-petitions') },
  ]);
