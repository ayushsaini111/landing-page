...
export async function generateMetadata() {
  ...
  const data = Data?.subServices?.landVerification ?? null;

  const title = data ? `${data.title} — Arshiv Legal` : 'Land Ownership & Title Verification — Arshiv Legal';
  const description =
    data?.description ||
    'Verify land ownership, title, past records, encumbrances and documents to ensure safe and dispute-free property transactions.';

  return {
    title,
    description,
    alternates: { canonical: canonicalize('/services/land-ownership-&-title-verification') },
    keywords: [
      'land verification',
      'title verification',
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${SEO_CONFIG.siteUrl}/images/land-verification-og.jpg`,
          width: 1200,
          height: 630,
    ...
    twitter: {
      ...
      title,
      description,
      images: [`${SEO_CONFIG.siteUrl}/images/land-verification-og.jpg`],
    },
...
const title = `${data.title} — Arshiv Legal`;
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: data.title, url: canonicalize('/services/land-ownership-&-title-verification') },
  ]);
