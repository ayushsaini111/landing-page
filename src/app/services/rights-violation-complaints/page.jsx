// /src/app/services/rights-violation-complaints/page.jsx
...
export async function generateMetadata() {
  ...
  const data = Data?.subServices?.rightsViolation ?? null;

  const title = data ? `${data.title} — Arshiv Legal` : 'Rights Violation Complaints — Arshiv Legal';
  const description =
    data?.description ||
    'File complaints and seek remedies when authorities or individuals violate your rights. We assist with documentation, filing complaints, and following up with the right channels.';

  return {
    title,
    description,
    alternates: { canonical: canonicalize('/services/rights-violation-complaints') },
    keywords: [
      'rights violation',
      'police misconduct',
      'authority misbehaviour',
      'complaint filing',
      'public rights',
      'Arshiv Legal Kanpur',
    ],
    openGraph: {
      title,
      description,
      url: canonicalize('/services/rights-violation-complaints'),
      siteName: SEO_CONFIG.siteName,
      type: 'article',
      images: [
        {
          url: `${SEO_CONFIG.siteUrl}/images/rights-violation-og.jpg`,
          width: 1200,
          height: 630,
          alt: 'Rights Violation Complaints - Arshiv Legal',
        },
      ],
    },
    twitter: {
      ...
      title,
      description,
      images: [`${SEO_CONFIG.siteUrl}/images/rights-violation-og.jpg`],
    },
...
export default async function Page() {
    // Import JSON at render time to avoid large module-level bundles
    const { default: Data } = await import('@/Data/data.json');
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: data.title, url: canonicalize('/services/rights-violation-complaints') },
  ]);
  ...
  const serviceSchema = {
    ...
    '@type': 'Service',
    name: data.title,
    serviceType: 'Rights Violation Complaints',
