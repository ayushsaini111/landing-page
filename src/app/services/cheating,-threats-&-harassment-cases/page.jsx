// /src/app/services/cheating,-threats-&-harassment-cases/page.jsx
...
export async function generateMetadata() {
  ...
  const data = Data?.subServices?.cheatingHarassment ?? null;

  const title = data ? `${data.title} — Arshiv Legal` : 'Cheating, Threats & Harassment Cases — Arshiv Legal';
  const description =
    data?.description ||
    'Advice and representation for cheating, threats, harassment and related criminal matters — from filing complaints to defending false allegations.';

  return {
    title,
    description,
    alternates: {
      canonical: canonicalize('/services/cheating,-threats-&-harassment-cases'),
    },
    keywords: [
      'cheating cases',
      'harassment cases',
      'threats legal help',
      'false allegation defence',
      'Arshiv Legal Kanpur',
    ],
    openGraph: {
      title,
      description,
      url: canonicalize('/services/cheating,-threats-&-harassment-cases'),
      siteName: SEO_CONFIG.siteName,
      type: 'article',
      images: [
        {
          url: `${SEO_CONFIG.siteUrl}/images/cheating-harassment-og.jpg`,
          width: 1200,
          height: 630,
          alt: 'Cheating, Threats & Harassment - Arshiv Legal',
        },
      ],
    },
    twitter: {
      ...
      title,
      description,
      images: [`${SEO_CONFIG.siteUrl}/images/cheating-harassment-og.jpg`],
    },
...
export default async function Page() {
 ...
 const title = `${data.title} — Arshiv Legal`;
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: data.title, url: canonicalize('/services/cheating,-threats-&-harassment-cases') },
  ]);
  ...
  const serviceSchema = {
    ...
    '@type': 'Service',
    name: data.title,
    serviceType: 'Cheating, Threats & Harassment Cases',
